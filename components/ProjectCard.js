...
const ProjectCard = ({ title, skills, description, imgSrc, centerImg, style, onClick, github }) => {
  ...
  return (
    <div
      className='w-full h-fit bg-space-cadet-light-trans rounded-3xl p-7 shadow-xl flex flex-col'
      style={style}
    >
      ...
      <div className='w-full h-[400px] max-lg:h-[280px] max-xm:h-[230px] max-md:h-[130px] max-sm:h-[90px] overflow-hidden rounded-2xl relative'>
        <div
          className={`absolute w-full h-[70%] bg-rose-bonbon bg-opacity-70 top-[15%] p-4 rounded-2xl flex items-center text-3xl hover:bg-opacity-80 transition-all shadow-2xl ${
            hover ? 'left-1' : 'left-8 max-sm:left-6'
        ...
        <div
          ...
          } ${hover ? 'left-14' : 'left-20 max-sm:left-16'}`}
        >
